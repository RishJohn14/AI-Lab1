...
def ucs_solver(maze):
    ...
    state = State(room=startRoom, parent=None, cost=0, priority=0)
    ...
    while not fr.is_empty():
        ...
        if(currentCoords==goalCoords):
            print("Path obtained via UCS Algorithm")
            fr.print_stats()
        ...
        for direction in currentRoom.get_connections():
            ...
            newState = State(room=newRoom, parent=currentState, cost=moveCost, priority=moveCost)
            fr.push(newState)
